...
export default function EditResume({setAlert, token, resume, setPage }) {
  ...
  useEffect(() => {
    if (resume) {
      ...
      setWorkExperience(resume.workExperience?.length ? resume.workExperience : [
        { company: "", joinYear: "", endYear: "", description: "" }
      ]);

      setProgrammingLanguages(resume.skills?.programmingLanguages?.join(", ") || "");
      setFrameworks(resume.skills?.frameworks?.join(", ") || "");
      setTools(resume.skills?.tools?.join(", ") || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await updateResume(resume._id, resumeData, token);
      setPage("app");
            setAlert({ show: true, type: "success", message: "Resume Updated Successfully" });
    
      console.log(res);

    } catch (err) {
      console.error(err);
           setAlert({ show: true, type: "success", message: "Failed To Update Resume" });
    }
  };
